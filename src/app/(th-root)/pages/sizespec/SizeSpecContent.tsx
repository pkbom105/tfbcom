...
// Map category ID (1-12) to buttonGroup ID
const categoryIdToGroupId: { [key: number]: string } = {
  1: "tshirt-standard",  // T-shirt
  2: "engi",             // Workshop shirt
  3: "maid",             // House Maid Uniform
  4: "pants",            // Pants
  5: "polo",             // Polo
  6: "engi",             // Engineer Jacket
  7: "chef",             // Chef Uniform
  8: "jacket",           // Vest (using jacket)
  9: "shirt",            // Shirt
  10: "jacket",          // Jacket
  11: "maid",            // Security Uniform
  12: "apron",           // Apron
};

export default function SizeSpecContent() {
  ...
  const getInitialTab = () => {
    if (categoryId) {
      ...
      return categoryIdToGroupId[numId] || buttonGroups[0].id;
    }
    return buttonGroups[0].id;
  };
  ...
  useEffect(() => {
    if (categoryId) {
      const numId = parseInt(categoryId, 10);
      const newTab = categoryIdToGroupId[numId] || buttonGroups[0].id;
      setActiveTab(newTab);
    }
  }, [categoryId]);
