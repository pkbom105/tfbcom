...
export default function PaymentPage({ lang = "th" }: { lang?: string }) {
  const isEn = lang === "en";
  const bankAccounts = [
    {
      bankName: isEn ? "Kasikorn Bank (K-Bank)" : "ธนาคารกสิกรไทย",
      accountName: isEn ? "Toffy Boutique Co., Ltd." : "บริษัท ทอฟฟี่ บูติก จำกัด",
      accountNumber: "090-2-40899-1",
      branch: isEn ? "Bangmod Branch" : "สาขาบางมด",
      color: "bg-green-600",
    },
    {
      bankName: isEn ? "Siam Commercial Bank (SCB)" : "ธนาคารไทยพาณิชย์",
      accountName: isEn ? "Toffy Boutique Co., Ltd." : "บริษัท ทอฟฟี่ บูติก จำกัด",
  ...
  const copyToClipboard = (num: string) => {
    ...
    alert(isEn ? "Account number copied: " + num : "คัดลอกเลขบัญชีแล้ว: " + num);
  };
    ...
    <main className="min-h-screen font-kanit bg-white pb-20">
      ...
      <section className="bg-slate-50 py-16 px-6 border-b-0 text-center">        
        <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-6">
            Payment | <span className="text-red-600">{isEn ? "Payment Information" : "การชำระเงิน"}</span>
        </h1>    
        <p className="text-slate-500 max-w-2xl mx-auto">
          {isEn ? "Check bank account details to proceed with your deposit payment." : "ตรวจสอบรายละเอียดเลขที่บัญชีธนาคารเพื่อดำเนินการชำระเงินมัดจำค่าสินค้า"}
        </p>
      </section>
      ...
      <section className="max-w-[1400px] mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-[35%_65%] gap-12 items-start">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-2xl font-bold text-slate-900">{isEn ? "Bank Transfer Channels" : "ช่องทางการโอนเงิน"}</h2>
          </div>
          ...
          <div className="bg-blue-50/50 border-l-4 border-blue-500 p-5 rounded-r-xl mt-10">
            <h4 className="text-blue-900 font-bold text-lg mb-2 flex items-center gap-2">
              <ExternalLink size={18} /> {isEn ? "Payment Terms" : "เงื่อนไขการชำระเงิน"}
            </h4>
            <ul className="text-md text-black-600 space-y-2 list-disc pl-4 leading-relaxed">
              <li>{isEn ? "50% deposit to confirm production" : "ชำระเงินมัดจำ 50% เพื่อยืนยันการผลิต"}</li>
              <li>{isEn ? "Remaining 50% balance before delivery" : "ชำระส่วนที่เหลือ 50% ก่อนจัดส่งสินค้า"}</li>
              <li>{isEn ? "Please send payment slip immediately after transfer" : "กรุณาแจ้งสลิปทันทีหลังโอนเงินเสร็จสิ้น"}</li>
            </ul>
          </div>
        </div>
        ...
        <div className="space-y-8">
          <div className="flex items-center justify-between">
             <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
               <Download className="text-red-600 w-6 h-6" /> {isEn ? "Account Summary Photo" : "ภาพสรุปเลขบัญชี"}
             </h2>             
          </div>
          ...
          <p className=" text-slate-400 text-lg italic">
            <a 
              ...
              >
              <Download size={24} />_ {isEn ? "Download Account Details" : "Download บันทึกเลขที่บัญชี"} 
              </a>
          </p>
        </div>
      </section>
