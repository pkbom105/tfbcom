...
export default function OurCustomerPage({ lang = "th" }: { lang?: string }) {
  const isEn = lang === "en";
  ...
  const customers = Array.from({ length: 57 }, (_, i) => {
    ...
    return {
      ...
      alt: isEn ? `Customer Logo No. ${fileNumber}` : `โลโก้ลูกค้า ทอฟฟี่ บูติก ลำดับที่ ${fileNumber}`,
    };
  });
      ...
      <section className="bg-slate-50 py-16 px-6 border-b">
        <div className="max-w-4xl mx-auto text-center space-y-6">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 leading-tight">
            {t("ลูกค้าที่ไว้วางใจเรา", "Our Valued Clients")}
          </h1>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 space-y-4 max-w-3xl mx-auto text-center">
            <p className="text-xl font-bold text-slate-800 leading-relaxed">
              {t("ความภูมิใจของเราที่ได้เป็นส่วนหนึ่งในการสร้างสรรค์ภาพลักษณ์ให้องค์กรชั้นนำ", "Our pride in crafting the professional image for leading organizations.")}
            </p>
            <p className="text-lg text-slate-600 leading-relaxed">
              {t("ทอฟฟี่ บูติก ขอขอบพระคุณบริษัท ห้างร้าน และหน่วยงานราชการต่างๆ ที่ไว้วางใจให้เราดูแลการผลิต", "Toffy Boutique would like to thank all companies, shops, and government agencies for trusting us.")} 
              <strong className="text-red-600 px-1">{t("ยูนิฟอร์มพนักงานและเสื้อโปโลคุณภาพ", "Quality Staff Uniforms and Polo Shirts")}</strong> 
              {t("เรามุ่งมั่นรักษามาตรฐานการผลิตเพื่อให้ลูกค้าได้รับสินค้าที่ดีที่สุดเสมอมา", "We are committed to maintaining production standards so customers always receive the best products.")}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-16">
        ...
        <div className="mt-24 text-center">
          <div className="bg-slate-900 rounded-3xl p-10 md:p-16 text-white shadow-2xl relative overflow-hidden">
            ...
            <h3 className="text-2xl md:text-3xl font-bold mb-6 relative z-10">{t("ร่วมเป็นส่วนหนึ่งของลูกค้าคนสำคัญของเรา", "Become one of our valued clients")}</h3>
            <p className="text-slate-400 mb-10 max-w-2xl mx-auto relative z-10">
              {t("ไม่ว่าจะเป็นองค์กรขนาดเล็กหรือใหญ่ ทอฟฟี่ บูติก พร้อมดูแลการผลิตชุดฟอร์มพนักงานให้ตอบโจทย์ทุกความต้องการของคุณ", "Whether a small or large organization, Toffy Boutique is ready to handle your staff uniform production to meet all your needs.")}
            </p>
            ...
            <button className="inline-flex items-center gap-3 bg-red-600 text-white px-12 py-5 rounded-full font-bold hover:bg-red-700 transition-all duration-300 group shadow-xl hover:-translate-y-1 relative z-10">
              {t("เริ่มสั่งผลิตกับเราวันนี้", "Start Your Production Today")} 
