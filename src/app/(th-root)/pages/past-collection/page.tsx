...
export default function PastCollectionPage({ lang = "th" }: { lang?: string }) {
  const isEn = lang === "en";
    ...
    <main className="min-h-screen bg-white font-kanit pb-20">
      ...
      <section className="bg-slate-50 py-16 px-6 border-b border-slate-100">
        <div className="max-w-5xl mx-auto text-center">
          <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-6">
            Sample Product  | <span className="text-red-600">{t("ตัวอย่างงานผลิตคุณภาพ", "Quality Production Portfolio")}</span>
          </h1>
          <div className="space-y-4 text-lg text-slate-600 leading-relaxed max-w-4xl mx-auto">
            <p className="font-semibold text-slate-800 text-xl">
              {t("ยูนิฟอร์มคุณภาพสูง จากความไว้วางใจของลูกค้าทั่วประเทศประสบการณ์กว่า 35 ปี", "High-quality uniforms trusted by clients nationwide with over 35 years of expertise.")}
            </p>
            <p>
              {t("ขอบคุณลูกค้าผู้มีอุปการะคุณที่ไว้วางใจให้", "Thank you to our valued customers for trusting")} <span className="text-red-600 font-bold">Toffy Boutique</span> {t("ดูแลงานผลิต", "with your production.")}
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="gallery-section" className="pt-16 max-w-7xl mx-auto px-6">
        <h2 className="text-2xl font-bold mb-10 border-l-4 border-red-500 pl-4 text-slate-900">
          {t("แคตตาล็อกผลงานผลิต", "Production Catalog")}
        </h2>
              ...
              <TabsContent key={tab.value} value={tab.value} className="mt-0 focus-visible:outline-none">
                ...
                >
                  <div className="mb-12 text-center">
                    <h3 className="text-3xl font-bold text-slate-800 mb-3">{t(tab.title, tab.titleEn)}</h3>
                    ...
                    {Array.from({ length: tab.count }).map((_, index) => {
                      ...
                      return (
                        <div key={index} className="flex flex-col gap-3">
                          ...
                          <div className="px-2">
                            <p className="font-bold text-slate-800 text-sm">{t("รหัสงาน", "Job ID")}: {imageNumber}</p>
                            <p className="text-xs text-slate-400">{t("หมวดหมู่", "Category")}: {t(tab.label, tab.labelEn)}</p>
                          </div>
                        </div>
                      );
                    })}
